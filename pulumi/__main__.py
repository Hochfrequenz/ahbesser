...
container = docker.Container("app-container",
...
container_group = azure_native.containerinstance.ContainerGroup("containergroup",
    ...
    ip_address=azure_native.containerinstance.IpAddressArgs(
        ports=[azure_native.containerinstance.PortArgs(port=80)],
        type="Public"
    )
)
...
custom_domain_binding = azure_native.web.CustomHostnameBinding("customdomainbinding",
...
github_actions_secret = github.ActionsSecret("azure_storage_connection_string",
...
pulumi.export("app_url", web_app.default_site_hostname.apply(lambda hostname: f"https://{hostname}"))
